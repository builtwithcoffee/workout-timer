...
assert.equal(managerNodes.filter(el=>el.tag==='button').length,0,'Starter rows have no selection or removal actions');
manager.closeWorkoutManager();
assert.deepEqual(manager.workoutFile(),beforeManager,'Done preserves the current workout');
manager.openWorkoutManager();
...
manager.openWorkoutManager();
managerNodes=manager.walk(manager.elements.get('#workoutManagerPacks'));
const rowRemove=managerNodes.find(el=>el.className==='manager-remove');
assert.equal(managerNodes.filter(el=>el.tag==='button').length,1,'Imported rows offer removal only');
rowRemove.handlers.click();
assert.equal(manager.packs.length,1,'Remove opens confirmation without deleting');
managerNodes=manager.walk(manager.elements.get('#workoutManagerPacks'));
managerNodes.find(el=>el.id==='packRemoveCancel').handlers.click();
assert.equal(manager.packs.length,1,'Cancel keeps the pack');
...
// Save failures during pack switching and removal leave the current imported edit intact.
