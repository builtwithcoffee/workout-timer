...
assert.doesNotMatch(html, /parts\.push\('Set ' \+ it\.set/);
assert.match(html, /\$\('#runSetCurrent'\)\.textContent = showRound \? it\.round : it\.set/);
assert.match(html, /const exerciseSkip = !!it\.exerciseId && !roundGroup && it\.blockMode !== 'emom'/);
assert.match(html, /exerciseSkip \? 'Skip exercise' : it\.phase === 'rest' \? 'Skip rest' : 'Skip'/);
...
assert.match(html, /id="btnEndWorkout" type="button">End workout<\/button>/);
assert.match(html, /<div class="runhead-actions">[\s\S]*?id="btnSkipBlock" type="button">Skip block<\/button>[\s\S]*?id="btnEndWorkout" type="button">End workout<\/button>/);
assert.doesNotMatch(html, /<div class="runctl">[\s\S]*?id="btnSkipBlock"/);
assert.match(html, /function skipCurrent\(\) \{[\s\S]*?delete S\.sessionSetResults\[setResultKey\(current\.exerciseId, current\.set\)\]/);
assert.match(html, /next = nextExerciseQueueIndex\(current\)/);
assert.match(html, /\$\('#btnSkip'\)\.addEventListener\('click',[\s\S]*?skipCurrent\(\)/);
assert.match(html, /const structuredWorkoutRun = S\.mode === 'workout' && S\.sessionPlan && S\.sessionPlan\.layout === 'blocks'/);
assert.match(html, /\$\('#btnEndWorkout'\)\.classList\.toggle\('hidden', amrapResult \|\| !structuredWorkoutRun\)/);
assert.match(html, /\$\('#btnEnd'\)\.classList\.toggle\('hidden', amrapResult \|\| structuredWorkoutRun\)/);
assert.match(html, /button\.textContent = workoutEnd \? 'End workout\?' : 'End\?'/);
